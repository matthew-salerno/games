...
class priorityQueue {
	constructor(reverse = false, arr = []) {
		if (reverse){
			this.cmp = function(a, b) {a.priority - b.priority};
		}
		else {
			this.cmp = function(a, b) {b.priority - a.priority};
		}
		this.arr = arr;
	...
	push(item, priority) {
		for (let i = this.arr.length; i >= 0; i--) {
			if (i === 0 || this.cmp(priority, this.arr[i-1].priority) > 0) {
				this.arr.splice(i, 0, {priority: priority, item: item});
			}
		}
	}
	pop() {
